 ...
 */
export async function generate48HourPredictions(location: {
  name: string;
  lat: number;
  lon: number;
  ...
  try {
    ...
    const historicalData = await fetchHistoricalDisasters(
      location.lat,
      location.lon
    );
 ...
 * Fetch historical disaster data
 */
async function fetchHistoricalDisasters(
  lat: number,
  lon: number
): Promise<any> {
  try {
...
Seismic Data:
${
  seismicData?.features
    ? `Recent earthquakes: ${seismicData.features.length}`
...
Generate 1-3 most likely predictions. Be realistic and data-driven. If no significant threats detected, return empty array [].`;
    ...
    return predictionsData.map((pred: any, index: number) => {
      const now = new Date();
      const peakTime = pred.peakTime
        ? new Date(pred.peakTime)
...
function generatePreparedness(
  ...
  const basePreparedness: EmergencyPreparedness = {
    ...
    supply24Hours: {
      ...
      food: {
        items: ["Non-perishable food", "Energy bars", "Canned goods"],
        priority: 2,
      },
      medical: {
        items: ["First aid kit", "Medications", "Bandages"],
        priority: 1,
      },
      tools: {
        items: ["Flashlight", "Battery radio", "Multi-tool"],
        priority: 2,
      },
      documents: {
        items: ["ID copies", "Insurance docs", "Emergency contacts"],
        priority: 1,
      },
      communication: {
        items: ["Charged phone", "Power bank", "Whistle"],
        priority: 1,
      },
      clothing: {
        items: ["Weather-appropriate clothes", "Sturdy shoes", "Rain gear"],
        priority: 3,
      },
    ...
    supply48Hours: {
      ...
      food: {
        items: ["3-day food supply", "Manual can opener", "Cooking supplies"],
        priority: 1,
      },
      medical: {
        items: [
      ...
      tools: {
        items: ["Generator/inverter", "Extra batteries", "Duct tape", "Rope"],
        priority: 2,
      },
      documents: {
        items: ["Full document copies", "Hard drive backup", "Legal papers"],
        priority: 1,
      },
      communication: {
        items: ["2-way radio", "Multiple chargers", "Signal mirror"],
        priority: 2,
      },
      clothing: {
        items: ["Complete change", "Blankets", "Sleeping bags"],
        priority: 2,
      },
      other: {
        items: ["Pet supplies", "Entertainment", "Comfort items"],
        priority: 3,
      },
    },
    emergencyContacts: [
      ...
      basePreparedness.immediateActions = [
        {
          ...
          description:
            "If flooding is imminent, move to upper floors or evacuate to higher elevation",
        ...
        },
        {
          ...
          description:
            "Shut off electricity and gas if water is entering your home",
    ...
    case "wildfire":
      basePreparedness.immediateActions = [
        ...
        },
        {
          ...
          description:
            "Shut all windows, doors, and vents to prevent embers from entering",
    ...
    case "heatwave":
      basePreparedness.immediateActions = [
        ...
        },
        {
          ...
          description:
            "Stay in air-conditioned buildings, visit cooling centers if needed",
        ...
        },
        {
          ...
          description:
            "Monitor elderly, children, and those with health conditions",
 ...
 */
function generateTemplatePredictions(location: {
  name: string;
  lat: number;
  lon: number;
}): DisasterPrediction[] {
